	...
	$.ajax({
		...
		success : function(data) {
			for(var i=0;i<data.length;i++){
				$("#shopClassifyDiv")
				.append("<span id='"+data[i].id+"' onclick='findShopByShopClassifyId("+data[i].id+")'>"+data[i].name+"</span>");
			}
		}
	});
	//获取第一页商店展示信息
...
function findShopByShopClassifyId(id){
	...
	for (var i = 0; i <= 8; i++) {
		...
		document.getElementById(id).className='layui-btn ayui-btn-radius layui-btn-normal';
	};
